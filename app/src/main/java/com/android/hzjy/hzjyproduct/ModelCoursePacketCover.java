...
//       CoursePacketTeachersInit(mCoursePacketInfo);
    ...
    public void getDataPacketageCollection(){
        ...
        paramsMap.put("stu_id", 1);    //学员id
        paramsMap.put("course_package_id", 2); //包id
        paramsMap.put("collection_status", 4);   //状态码
                        ...
                        collectionBean collectionBean = response.body();
                            ...
                            if (code==200){
                              ...
                              if (message.equals("success")){
                                  Toast.makeText(mControlMainActivity, "收藏成功", Toast.LENGTH_SHORT).show();
                              }else {
    ...
    public void getDataPacketageTeacher() {
        ...
        String strEntity = gson.toJson(paramsMap);
        RequestBody body = RequestBody.create(okhttp3.MediaType.parse("application/json;charset=UTF-8"), strEntity);
        modelObservableInterface.queryCoursePackageTeacher(body)
                .enqueue(new Callback<CourseTeacherBean>() {
                    @Override
                    public void onResponse(Call<CourseTeacherBean> call, Response<CourseTeacherBean> response) {
                        CourseTeacherBean body1 = response.body();
                        int code = body1.getCode();
                        if (code == 200) {
                            List<CourseTeacherBean.DataBean> dataBeans = body1.getData();
                            for (int i = 0; i < dataBeans.size(); i++) {
                                ...
                                mCoursePacketInfo.mTeacherInfoList.get(i).mTeacherName = true_name;
                                mCoursePacketInfo.mTeacherInfoList.get(i).mTeacherMessage = introduction;
                                mCoursePacketInfo.mTeacherInfoList.get(i).mTeacherCover = head;
                                mCoursePacketInfo.mTeacherInfoList.get(i).mTeacherID = String.valueOf(user_id);
                            }
                        }else {
                            Log.e(TAG, "onResponse: "+code);
                        }
                    ...
                    @Override
                    public void onFailure(Call<CourseTeacherBean> call, Throwable t) {
                        Log.e(TAG, "onFailure: " + t.getMessage());
                    }
                });
    }
                ...
                if (body1 != null) {
                    ...
                    if (code == 200) {
                        ...
                        for (int i = 0; i < courseListInfo.size(); i++) {
                            ...
                            mCoursePacketInfo.mStageCourseInfoList.get(i).mStageCourseName = course_name;
                            mCoursePacketInfo.mStageCourseInfoList.get(i).mStageCourseId = String.valueOf(course_id);
                        ...
                        for (int i = 0; i < stageListInfo.size(); i++) {
                            ...
                            mCoursePacketInfo.mStageCourseInfoList.get(i).mStageCourseOrder= String.valueOf(sort_of_stage);
                            mCoursePacketInfo.mStageCourseInfoList.get(i).mStageCourseId= String.valueOf(stage_id);
                            mCoursePacketInfo.mStageCourseInfoList.get(i).mStageCourseProgrProgress= String.valueOf(stageLearningProgress);
                        }
                    }
                }
            }
    ...
    public void getDataPacketDetails() {
        ...
        dataPacketDetailsBeanCall.enqueue(new Callback<DataPacketDetailsBean>() {
            ...
            public void onResponse(Call<DataPacketDetailsBean> call, Response<DataPacketDetailsBean> response) {
                 ...
                 */
                String details = dataPacketDetailsBean.getDetails();//课程包简介
                int collection_status = dataPacketDetailsBean.getCollection_status();//收藏状态，1代表收藏，2代表没有收藏
                int course_package_id = dataPacketDetailsBean.getCourse_package_id();//课程包id
                String describe = dataPacketDetailsBean.getDescribe();//课程包详情
                //赋值
                ...
                //收藏的状态
                mCoursePacketInfo.mCoursePacketCourseNum = String.valueOf(collection_status);
            }
